-- ВСЕ НАСТРОЙКИ, КОД И ИНТЕРФЕЙС НАХОДЯТСЯ ЗДЕСЬ СТРОГО ВНУТРИ ОКНА
local FOLDER, OFFSET, SMOOTH = "Workplace", Vector3.new(6, 1.5, 4), 0.1
local Players, RunService, player = game:GetService("Players"), game:GetService("RunService"), game:GetService("Players").LocalPlayer
...
-- 1. СОЗДАЕМ ТЕЛЕВИЗОР С НЕОНОВОЙ СВЕТЯЩЕЙСЯ РАМКОЙ ПО КРАЯМ
...
-- Неоновая обводка (рамка) спереди по контуру
local border = Instance.new("SelectionBox")
border.Adornee = screen
border.Color3 = customColor
border.LineThickness = 0.04 -- Аккуратная тонкая линия по краям
border.SurfaceColor3 = customColor
border.Visible = true
...
-- 2. ИНТЕРФЕЙС УПРАВЛЕНИЯ (ЛИШНИЙ НАКЛАДЫВАЮЩИЙСЯ ТЕКСТ УБРАН)
...
local function up() lbl.Text = "Играет:\n" .. (playlist[index]:match("([^/]+)$") or "Стандартный трек") end up()
...
local btnP = createBtn("⏸", 0.38, 0.22, UDim2.new(0,60,0,45), Color3.fromRGB(0,150,255)) btnP.TextSize = 22
...
local btnNeon = createBtn("Материал: Обычный", 0.08, 0.48, UDim2.new(0,216,0,30), Color3.fromRGB(60,60,60)) btnNeon.TextSize = 13

-- 3. РАДУЖНАЯ ПОЛОСКА ВЫБОРА ЦВЕТА
local colorSlider = Instance.new("ImageButton")
colorSlider.Size = UDim2.new(0, 216, 0, 20)
colorSlider.Position = UDim2.new(0.08, 0, 0.74, 0)
colorSlider.BorderSizePixel = 0
colorSlider.Parent = panel
...
picker.Size = UDim2.new(0, 6, 0, 24)
picker.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
picker.Position = UDim2.new(0.33, -3, 0, -2) 
picker.Parent = colorSlider
...
local function updateColorFromSlider(X)
    ...
    local width = colorSlider.AbsoluteSize.X
    local percentage = math.clamp(rX / width, 0, 1)
    
    picker.Position = UDim2.new(percentage, -3, 0, -2)
    customColor = Color3.fromHSV(percentage, 1, 1)
    
    -- Синхронизируем цвет рамки с выбранным цветом слайдера
    border.Color3 = customColor
    border.SurfaceColor3 = customColor
...
-- ЛОГИКА НАЖАТИЙ НА КНОПКИ ПАНЕЛИ
toggleButton = btn.MouseButton1Click:Connect(function() panel.Visible = not panel.Visible btn.BackgroundColor3 = panel.Visible and Color3.fromRGB(0, 150, 255) or Color3.fromRGB(35, 35, 35) end)
playPauseBtn = btnP.MouseButton1Click:Connect(function() isPlaying = not isPlaying if isPlaying then sound:Resume() btnP.Text = "⏸" else sound:Pause() btnP.Text = "▶" end end)
prevBtn = btnB.MouseButton1Click:Connect(function() index = index - 1 if index < 1 then index = #playlist end play(index) up() end)
nextBtn = btnN.MouseButton1Click:Connect(function() index = index + 1 if index > #playlist then index = 1 end play(index) up() end)

btnNeon.MouseButton1Click:Connect(function()
    isNeon = not isNeon
    if isNeon then
        screen.Material = Enum.Material.Neon
        screen.Color = customColor
        btnNeon.Text = "Материал: Неоновый светится"
        btnNeon.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
    else
        screen.Material = Enum.Material.SmoothPlastic
        screen.Color = Color3.fromRGB(15, 15, 15)
        btnNeon.Text = "Материал: Обычный пластик"
        btnNeon.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    end
end)

-- 4. ПОЛЕТ ТЕЛЕВИЗОРА ЗА ИГРОКОМ
...
-- 5. ЭФФЕКТЫ БИТА (Размер телевизора строго зафиксирован)
RunService.Heartbeat:Connect(function()
    ...
    for i, b in ipairs(bars) do
        ...
        b.Size, b.Position, b.BackgroundColor3 = UDim2.new(b.Size.X.Scale,0,h,0), UDim2.new(b.Position.X.Scale,0,0.95-h,0), customColor
    end
end)
